...
import com.mgc.letobox.happy.floattools.components.playgametask.rxdownload4.utils.sliceCount
import okio.*
...
class RangeTmpFile(private val tmpFile: File) {
    ...
    private val bufferedSink = Okio.buffer(Okio.sink(tmpFile))
    private val bufferedSource = Okio.buffer(Okio.source(tmpFile))
    ...
    class FileHeader(
        ...
        fun write(sink: BufferedSink, totalSize: Long, totalSegments: Long) {
            ...
            sink.apply {
                write(ByteString.decodeHex(FILE_HEADER_MAGIC_NUMBER))
                writeLong(totalSize)
