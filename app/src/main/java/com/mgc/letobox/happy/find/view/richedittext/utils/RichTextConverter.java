 ...
 */
public class RichTextConverter implements ContentHandler {
    ...
    public RichTextConverter(Context ctx, IEmojiFactory emojiFactory) {
        _ctx = ctx;
    ...
    public static Spanned fromRichText(Context ctx, String richText, IEmojiFactory iEmojiFactory) {
        return new RichTextConverter(ctx, iEmojiFactory).convert(richText);
    }
    ...
    private void startImg(Attributes attributes) {
        ...
        if (!"yiqiFace".equals(classString)) {
            ...
            // emoji
            EmojiSpan emojiSpan = new EmojiSpan(_ctx,
