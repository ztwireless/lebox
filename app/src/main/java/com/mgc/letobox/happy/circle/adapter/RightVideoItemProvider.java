...
public class RightVideoItemProvider extends BaseItemProvider<CircleTieZiListResponse, BaseViewHolder> {

    private CircleDetailsAdapter.CircleDetailsClickListener listener;

    public RightVideoItemProvider(CircleDetailsAdapter.CircleDetailsClickListener listener) {
        this.listener = listener;
    }

    @Override
    public int viewType() {
        return CircleListAdapter.RIGHT_VIDEO_NEWS;
    }
    ...
    public void convert(final BaseViewHolder helper, final CircleTieZiListResponse data, final int position) {
        ...
        ((TextView) helper.getView(R.id.tv_duration)).setVisibility(View.VISIBLE);
        ((LinearLayout) helper.getView(R.id.ll_duration)).setVisibility(View.VISIBLE);
